...
function AIView({ entries, tasks, categories, reports, setReports, aiHistory, focus, commitments, prayerLog, religious }) {
  ...
  async function generate() {
    ...
    try {
      ...
      // Fallback محلي: إحصائيات حقيقية مباشرة من البيانات المخزّنة، بدون اتصال بأي خدمة
    ...
    <div style={S.view}>
      ...
      {report && !report.error && (
        <div style={S.reportCard}>
          ...
          <div style={S.reportBlock}><div style={S.reportBlockTitle}>{report.local ? "إحصائيات اليوم" : "رؤى"}</div>{report.insights?.map((it, i) => <div key={i} style={S.reportItem}><span style={S.reportDot}>•</span>{it}</div>)}</div>
          {!report.local && <div style={S.reportBlock}><div style={S.reportBlockTitle}>توصيات</div>{report.recommendations?.map((it, i) => <div key={i} style={S.reportItem}><span style={S.reportDot}>•</span>{it}</div>)}</div>}
...
// مهمة دينية فيها عدّاد زمني مدمج
function ReligiousTask({ task, onUpdate, onRemove, addPoints, showToast }) {
  ...
  return (
    <div style={{ ...S.religiousItem, ...(task.done ? S.religiousItemDone : {}) }}>
      <div style={S.religiousTop}>
        <div style={{ flex: 1 }}>
          <div style={S.religiousTitle}>{task.title}</div>
          <div style={S.religiousMeta}>
      ...
      {!task.done && (
        <div style={S.timerControlsRow}>
          <button onClick={() => setRunning((r) => !r)} style={S.miniTimerBtn}>
            {running ? <><Pause size={14} /> إيقاف</> : <><Play size={14} /> {seconds > 0 ? "متابعة" : "ابدأ العدّاد"}</>}
          </button>
          <button onClick={finish} disabled={seconds === 0} style={{ ...S.miniDoneBtn, ...(metTarget && seconds > 0 ? S.miniDoneBtnReady : {}) }}>
            <CheckCircle2 size={14} /> تم
          </button>
        </div>
      )}
      {task.done && <div style={S.religiousDoneRow}><CheckCircle2 size={15} color="#5FA8A0" /> مكتملة</div>}
    </div>
  );
}
...
function FocusView({ focus, setFocus, commitments, setCommitments, categories, entries, addPoints, showToast }) {
  ...
  const sessionRef = useRef(null); // { startedAt, targetMinutes, label, isStudy }

  // عند فتح الصفحة: تحقق من وجود جلسة نشطة محفوظة، واحسب الوقت المنقضي الحقيقي
  useEffect(() => {
    (async () => {
  ...
  function pauseTimer() {
    // الإيقاف المؤقت ينهي تتبّع الاستمرارية الزمنية الحقيقية (لتفادي حساب وقت غير حقيقي)
    setRunning(false);
  ...
  // تسجيل يدوي مباشر بعدد الدقائق (بدون تايمر)
  ...
  // أي نشاط من السجل اليومي بفئة "دراسة" يُحسب تلقائياً ضمن تقرير الدراسة
      ...
      {subTab === "timer" && (
        <>
          <div style={S.modeToggleRow}>
            <button onClick={() => setManualMode(false)} style={{ ...S.modeToggleBtn, ...(!manualMode ? S.modeToggleBtnActive : {}) }}><Timer size={13} /> عدّاد تلقائي</button>
            <button onClick={() => setManualMode(true)} style={{ ...S.modeToggleBtn, ...(manualMode ? S.modeToggleBtnActive : {}) }}><Edit3 size={13} /> إدخال يدوي</button>
          </div>
          ...
          {!manualMode ? (
            ...
            <div style={S.timerCard}>
              <div style={S.manualEntryRow}>
                <input
                  type="number" inputMode="numeric" value={manualMinutes}
                  onChange={(e) => setManualMinutes(e.target.value)}
                  style={S.manualInput} placeholder="25"
                />
                <span style={S.manualUnit}>دقيقة</span>
              </div>
              <div style={S.studyToggleRow}>
...
// تقرير تركيز قابل لإعادة الاستخدام (دراسة أو عام)
...
// روبوتات التحدي الثلاثة
function BotsChallenge({ focus }) {
  const [tick, setTick] = useState(0);
  // يتحدث كل دقيقة ليعطي إحساس الحركة الحية
  useEffect(() => {
  ...
  // تقدم الروبوتات يعتمد على الساعة الحالية ويتفاعل مع تقدمك
  const hour = new Date().getHours() + new Date().getMinutes() / 60;
  const bots = useMemo(() => {
    // الناجح: منظم، يبدأ مبكر ويتقدم بثبات، ويتفوق عليك إن توقفت
...
function AchieveView({ achieve, setAchieve, profile, focus, tasks, prayerLog, religious, addPoints, showToast }) {
  ...
  async function askCoach(mood) {
    ...
    try {
      ...
      // الوضع الذكي غير متاح: نستخدم رداً محلياً جاهزاً فوراً بدل ترك الخانة فارغة
      setSmartUnavailable(true);
  ...
  async function generate(kind) {
    ...
    try {
      ...
      // Fallback محلي: لا تترك الخانة فارغة، اعرض مهاماً جاهزة فوراً مرتبطة بهوايات المستخدم
      setSmartUnavailable(true);
  ...
  // لا تترك الخانة فارغة أبداً: إن لم توجد عناصر بهذا التبويب، عبّئها فوراً من البنك المحلي
  useEffect(() => {
  ...
  return (
    <div style={S.view}>
      ...
      {smartUnavailable && (
        <div style={S.smartBanner}>
          <Zap size={14} color="#C9A24B" />
          <span>الوضع الذكي غير متاح الآن، نعرض لك مهام جاهزة لحين تفعيل الذكاء الاصطناعي.</span>
        </div>
      )}
