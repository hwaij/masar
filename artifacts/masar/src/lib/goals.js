...
// - أسبوعي: 7 أيام من الإنشاء، مراجعة واحدة في اليوم الأخير (نهاية الأسبوع).
// - شهري: من يوم الإنشاء حتى نهاية شهره التقويمي، مراجعة واحدة في آخر يوم
//   من الشهر.
...
//
// خلل حقيقي وُجد وأُصلح: كانت مراجعة الأسبوعي/الشهري الوحيدة تقع في
// *منتصف* الفترة (اليوم الرابع/الخامس عشر) بينما السؤال المعروض فعلياً
// للمستخدم هو "هل حققت هدفك؟" (سؤال إنجاز نهائي لا سؤال تقدّم) - ولأن هذه
// المراجعة الوحيدة هي أيضاً "الأخيرة" برمجياً (checkpoints.length===1)، أي
// إجابة "لا" عندها كانت تُغلق الهدف بحالة "فشل" نهائياً، رغم بقاء نصف
// الأسبوع أو نصف الشهر فعلياً لإنجازه. هذا يناقض حتى نمط الهدف السنوي في
// نفس الملف (حيث المراجعات الوسيطة لا تُغلق الهدف، فقط المراجعة الأخيرة
// الحقيقية في نهاية الفترة). الإصلاح: مراجعة الأسبوعي/الشهري الوحيدة تقع
// الآن في اليوم الأخير الفعلي من الفترة، بما يطابق سؤالها ومعناها.
export function buildGoalPlan(period, createdDateKey) {
  if (period === "weekly") {
    const cells = Array.from({ length: 7 }, (_, i) => addDaysLocal(createdDateKey, i));
    const checkpoints = [addDaysLocal(createdDateKey, 6)];
  ...
  if (period === "monthly") {
    ...
    const checkpoints = [addDaysLocal(som, total - 1)];
    return { cells, checkpoints, unit: "day" };
  }
  // yearly
