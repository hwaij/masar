...
export default function MasarApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // خلل حقيقي وُجد وأُصلح (نفس فئة خلل onAuthChange المُصلَح بـ333cd50،
      // لكن هنا بمسار التحميل الأول لا onAuthChange): كانت الحماية أعلاه
      // تُفعَّل فقط عند timedOut=true (قفل عالق يعلَق النداء)، لكن نفس فئة
      // القفل يمكنها أيضاً أن تُعيد إجابة "لا جلسة" سريعة لكن خاطئة (بلا
      // تعليق فعلي) - خصوصاً بعد إعادة تحميل الصفحة بالكامل التي يفرضها
      // المتصفح أحياناً بعد بقاء تبويب/PWA بالخلفية طويلاً على الجوال (لا
      // مجرد visibilitychange، بل mount كامل جديد يمر بهذا المسار لا
      // onAuthChange). إن كان هذا صحيحاً، كانت هذه الحالة (!timedOut) تُهمَل
      // تماماً وتُعامَل كتسجيل خروج حقيقي فوراً - فيتأثر بها كل ما يعتمد على
      // CURRENT_OWNER (المظهر، النقاط/المستوى gamify، المهام، الأهداف، وكل
      // بيانات المستخدم السحابية دون استثناء، فكلها تمر بنفس نقطة الاختناق
      // هذه). تسجيل خروج حقيقي فعلي يمسح الجلسة المخزَّنة محلياً أصلاً (انظر
      // signOut بـauth.js) فلا خطر من فقدان حالة "ضيف حقيقي" هنا - الاعتماد
      // على وجود جلسة محلية مخزَّنة كإشارة أوثق من مجرد رد الشبكة اللحظي.
      if (!u) {
        const cached = getCachedSessionUser();
        if (cached) u = cached;
        else if (timedOut) setSessionCheckAmbiguous(true);
      }
