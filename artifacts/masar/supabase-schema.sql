...
-- تتبّع دقيق لتقدّم الجولة التفاعلية الجديدة (Guided Onboarding): يُبنى
-- فوق tour_seen الموجود أعلاه بلا تغيير معناه أو شرط ظهوره إطلاقاً
-- (tour_seen يبقى العلم الوحيد لـ"الجولة الأساسية اكتملت أو تُخطّيت"،
-- ونفس سلوكه القديم للمستخدمين الحاليين). هذا العمود يضيف فقط ما لم يكن
-- ممكناً بعلم منطقي واحد: استئناف الجولة الأساسية من نفس الخطوة إن أغلق
-- المستخدم التطبيق في المنتصف (core.step)، وحالة كل جولة سياقية مستقلة
-- لكل قسم (modules.<name>.done) تُعرض تلقائياً أول مرة يُفتح فيها ذلك
-- القسم فعلياً - لكل المستخدمين القدامى والجدد على حدٍّ سواء بما أن هذا
-- العمود يبدأ فارغاً {} للجميع. أي مفتاح غير موجود في الـjsonb يُقرأ في
-- الكود كـ false/0 افتراضياً - لا حاجة لأي ترحيل بيانات.
