...
-- تتبّع دقيق لتقدّم الجولة التفاعلية (MASAR JOURNEY، رحلة واحدة مترابطة
-- عبر التطبيق مقسّمة لمراحل): يُبنى فوق tour_seen الموجود أعلاه بلا تغيير
-- معناه أو شرط ظهوره إطلاقاً (tour_seen يبقى العلم الوحيد لـ"الرحلة
-- اكتملت أو تُخطّيت"، ونفس سلوكه القديم للمستخدمين الحاليين - لا تظهر لهم
-- الرحلة الجديدة تلقائياً أبداً، فقط عبر "إعادة تشغيل الجولة" الاختيارية).
-- core.{stage,step} يحفظ المرحلة والخطوة الحاليتين معاً لاستئناف الرحلة من
-- نفس النقطة تماماً إن أُغلق التطبيق في المنتصف (بديل عن الشكل الأقدم
-- core.{step} فقط، من الجولة الأساسية السابقة ذات الـ6 خطوات - أي بيانات
-- قديمة بهذا الشكل تُقرأ بأمان كبداية جديدة من المرحلة الأولى، لا كموضع
-- خاطئ). modules.<name>.done تبقى كما هي لحالة كل جولة سياقية مستقلة لكل
